...
-- NOTE: Les indices seront créés par V10 si les colonnes existent
-- Ici on essaie de créer les indices, mais ils peuvent échouer si les colonnes n'existent pas
DO $$
BEGIN
...
-- Commentaires (peuvent échouer si colonnes manquent)
DO $$
BEGIN
...
-- Données de test (optionnel, peuvent échouer si colonnes manquent)
DO $$
BEGIN
