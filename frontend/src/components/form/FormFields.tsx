  ...
  return (
    ...
    >
      {errorList.map(([field, error]) => (
        <div key={field} style={{ color: colors.danger[700], fontSize: '0.875rem' }}>
          <strong>{field}:</strong> {getErrorText(error)}
        </div>
      ))}
    </Box>
  )
      ...
      {title && (
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, gap: 1 }}>
          ...
          <div>
            ...
            {subtitle && <Box sx={{ fontSize: '0.875rem', color: colors.gray[600] }}>{subtitle}</Box>}
          </div>
        </Box>
      )}
      <Box
