...
const LandingFeatures = ({ features, workflow, useCases, capabilities }: LandingFeaturesProps) => {
  return (
    <>
        ...
        <Container maxWidth="lg">
          <div className="grid md:grid-cols-3 gap-4">
            {capabilities.map((item, index) => (
              <div key={index} className="rounded-xl border border-primary-100 bg-primary-50/70 p-5">
      ...
      {/* Features Section */}
      <section id="features" className="py-20 bg-primary-25">
        <Container maxWidth="lg">
          <motion.div
            initial="hidden"
            whileInView="visible"
              ...
              >
                <div className="flex flex-col md:flex-row gap-8">
                  <div className="flex-shrink-0">
                    <div className="w-16 h-16 bg-primary-100 rounded-xl flex items-center justify-center text-primary-600 border border-primary-200">
                      {feature.icon}
                    </div>
                  </div>
          ...
          >
            {workflow.map((item, index) => (
              <motion.div key={index} variants={fadeIn} className="flex flex-col items-center text-center flex-1">
                <div className="w-16 h-16 bg-primary-600 text-white rounded-full flex items-center justify-center text-2xl font-bold mb-3">
                  {item.step}
                </div>
                <div className="text-2xl text-primary-600 mb-2">{item.icon}</div>
        ...
        <Container maxWidth="lg">
          ...
          >
            {useCases.map((useCase, index) => (
              ...
              >
                <div className="text-4xl text-primary-600 mb-4">{useCase.icon}</div>
