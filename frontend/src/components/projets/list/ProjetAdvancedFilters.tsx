...
interface ProjetAdvancedFiltersProps {
  filters: ProjetFilterState
  onFiltersChange: (filters: ProjetFilterState) => void
  chefProjets: string[]
}
...
const ProjetAdvancedFilters = ({
  filters,
  onFiltersChange,
  chefProjets,
}: ProjetAdvancedFiltersProps) => {
  const [anchorEl, setAnchorEl] = useState<HTMLButtonElement | null>(null)
  const open = Boolean(anchorEl)

  const activeCount = Object.entries(filters).filter(([, v]) => v !== '').length

  const handleChange = (key: keyof ProjetFilterState, value: string) => {
  ...
  const getActiveFilterTags = (): { key: string; label: string }[] => {
    ...
    if (filters.avancementMin || filters.avancementMax) {
      const min = filters.avancementMin || '0'
      const max = filters.avancementMax || '...'
      tags.push({ key: 'avancement', label: `Avancement: ${min}% - ${max}%` })
    }
    ...
    if (filters.chefProjet) {
      tags.push({ key: 'chefProjet', label: `Chef: ${filters.chefProjet}` })
    }
    return tags
  }
  ...
  const removeFilter = (key: string) => {
    ...
    switch (key) {
      ...
      case 'avancement': updated.avancementMin = ''; updated.avancementMax = ''; break
      case 'date': updated.dateDebutFrom = ''; updated.dateDebutTo = ''; break
      case 'chefProjet': updated.chefProjet = ''; break
    }
        ...
        <Box sx={{ p: 2 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1, mb: 1.5 }}>
            <TextField
              ...
              onChange={(e) => handleChange('avancementMin', e.target.value)}
              sx={{ flex: 1 }}
            ...
            <TextField
              ...
              onChange={(e) => handleChange('avancementMax', e.target.value)}
              sx={{ flex: 1 }}
              inputProps={{ step: 5, min: 0, max: 100 }}
            />
          </Box>
          ...
          {chefProjets.length > 0 && (
            <TextField
              ...
              onChange={(e) => handleChange('chefProjet', e.target.value)}
            >
              <MenuItem value="">Tous</MenuItem>
              {chefProjets.map((name) => (
...
export default ProjetAdvancedFilters
