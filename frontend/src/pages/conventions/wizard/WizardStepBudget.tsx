...
const emptyLigne = (tauxCommission: number): BudgetLigne => ({
  designation: '',
  montantHT: 0,
  tauxTVA: 20,
  montantTTC: 0,
...
const WizardStepBudget = ({ formData, setFormData, handleChange, totals }: WizardStepBudgetProps) => {
  ...
  const [newLigne, setNewLigne] = useState<BudgetLigne>(emptyLigne(formData.tauxCommission))
  ...
  const handleAddLigne = () => {
    ...
    setFormData((prev: ConventionWizardFormData) => ({
      ...prev,
      lignesBudget: [...prev.lignesBudget, { ...newLigne, montantTTC }],
    }))
    setNewLigne(emptyLigne(formData.tauxCommission))
      ...
      <Box>
        ...
        <Card sx={{ ...componentStyles.card, p: 2, mb: 2 }}>
          ...
          <Box sx={{
            ...
            gridTemplateColumns: {
              xs: '1fr',
              md: isParCategorie ? '1fr 0.6fr 1fr 1fr 0.6fr' : '1fr 0.6fr 1fr',
            },
            gap: 1.5,
            alignItems: 'flex-start',
          }}>
            <DecimalInput size="small" label="Montant HT" value={newLigne.montantHT}
              onChange={(v) => setNewLigne({ ...newLigne, montantHT: v, montantTTC: v * (1 + newLigne.tauxTVA / 100) })}
              decimalPlaces={2} min={0} sx={{ minWidth: 140 }}
            />
            <DecimalInput size="small" label="TVA (%)" value={newLigne.tauxTVA}
              onChange={(v) => setNewLigne({ ...newLigne, tauxTVA: v, montantTTC: newLigne.montantHT * (1 + v / 100) })}
              decimalPlaces={2} min={0} max={100} sx={{ minWidth: 80 }}
            />
            <DecimalInput size="small" label="Montant TTC" value={newLigne.montantTTC}
