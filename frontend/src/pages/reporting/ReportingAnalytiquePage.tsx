...
export default function ReportingAnalytiquePage() {
  ...
  const handleExportExcel = async () => {
    ...
    if (viewMode === 'simple') {
      ...
      // Ajouter les données
      rows.forEach(row => {
        const rowData: Record<string, number | string> = { [getDimensionName(selectedDim1)]: row }
        cols.forEach(col => {
      ...
      // Ligne des totaux
      const totalRow: Record<string, number | string> = { [getDimensionName(selectedDim1)]: 'Total' }
      cols.forEach(col => {
  ...
  const getTotal = () => {
    if (viewMode === 'simple') {
      ...
      return aggregation2D.reduce((sum, row) => sum + parseFloat(String(row.montant)), 0)
    }
  }
  ...
  const createCrossTable = () => {
    ...
    aggregation2D.forEach((item) => {
      ...
      data[item.dimension1][item.dimension2] = parseFloat(String(item.montant))
    })
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={getChartData()}>
                    ...
                    <YAxis />
                    <Tooltip
                      formatter={(value: number | string) => formatMontant(Number(value) || 0)}
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
                  <PieChart>
                    ...
                    <Tooltip formatter={(value: number | string) => formatMontant(Number(value) || 0)} />
                    <Legend />
                  </PieChart>
                </ResponsiveContainer>
              </CardContent>
